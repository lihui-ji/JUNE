...
int Cell :: direction(unsigned indFace) const{
    int indCellFace = cellFaceIndices.lookUp (indFace);
    if (indCellFace == -1){
        throw std::invalid_argument("The indFace is not in the cell.");
    }
    return cellFaceDirections[indCellFace];
}
double Cell :: distance(unsigned indFace) const{
    int indCellFace = cellFaceIndices.lookUp (indFace);
    if (indCellFace == -1){
        throw std::invalid_argument("The indFace is not in the cell.");
    }
